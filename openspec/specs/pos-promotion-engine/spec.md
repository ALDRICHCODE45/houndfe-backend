...
#### Scenario: CATEGORIES targeting matches by category id (DEFERRED — not supported by the current engine)
...
> **DEFERRED — not part of the current engine contract.** `CATEGORIES` / `BRANDS` engine targeting is **NOT** activated by the current engine. `isSupportedEngineType` intentionally returns `false` for these types, and `matchTargetTier` returns `null` for `CATEGORIES` / `BRANDS` (unit-tested negative case in `match-target-tier.spec.ts`). This scenario documents FUTURE intended behavior only; it is covered by a separate "activate deferred promo types" backlog item. **Do not infer CATEGORIES support from the presence of this scenario** — the engine will reject or silently skip such promotions today.
...
- `src/promotions/application/match-target-tier.spec.ts` (table-driven: VARIANT/PRODUCT/null tier; null `variantId` never matches VARIANTS; CATEGORIES/BRANDS → null)
