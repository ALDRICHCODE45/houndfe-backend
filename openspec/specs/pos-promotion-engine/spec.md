...
## Batch Delete (batch-delete — archived 2026-07-24)

### R11: Batch Delete Endpoint

`POST /promotions/batch-delete` MUST accept `{ ids: string[] }` and delegate to the `BatchDeleteOrchestrator`. The endpoint MUST be guarded by `@RequirePermissions(['batch_delete', 'Promotion'])` and `BatchDeleteGuard`.
...
- THEN the entire batch is rejected; response `409 { code: 'PROMOTION_REFERENCED_BY_SALE', offendingIds: [p2], reason }`; none deleted
...
### R13-R15: Cascade, State-Agnostic, Service Contract

Join-table rows (PromotionTargetItem, PromotionCustomer, PromotionPriceList, PromotionDayOfWeek) MUST cascade on delete. Promotion state MUST NOT block batch delete. `PromotionsService` implements `BatchDeletableService` contract.
...
## R16: Quotations Slice — `Context` Discriminant (merged from `changes/quotations/specs/pos-promotion-engine/delta.md`)
...
### Verification Surface (Quotations slice)
