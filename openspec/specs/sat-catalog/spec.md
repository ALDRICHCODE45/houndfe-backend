# SAT Catalog Specification

## Purpose

Define the server-backed reference catalog for the Mexican SAT `c_ClaveProdServ` keys used by `Product.satKey`. The catalog exposes a typeahead search and a single-key lookup, and it enforces a strict validation contract on product create/update so that only catalog-known SAT keys can land on a product.
