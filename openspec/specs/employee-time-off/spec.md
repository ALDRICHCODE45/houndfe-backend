# Employee Time-Off Specification

## Purpose

Define the HR employee time-off bounded context: request, atomic request +
outbox emission, review (approve / reject), tenant-wide pending-approvals
inbox, medical-reason visibility, cancellation, single-employee listing, and
vacation balance. Authority is decoupled into three independent levers —
**REQUEST** is gated by `create:EmployeeTimeOff`; **APPROVE / REJECT**
authority is the `update:EmployeeTimeOff` permission and is tenant-wide
(no org-chart filter, no manager identity); **NOTIFY** is owned by the
`notification-config` capability (see `time-off-notifications`). The
historical `Employee.userId` bridge that connected the employee record to a
manager user has been retired; the former manager-scoped pending-approvals
routing is replaced by a tenant-wide inbox. No new CASL permissions are
introduced — existing `create / update / read:EmployeeTimeOff` +
`read:EmployeeTimeOffMedical` cover all levers.
