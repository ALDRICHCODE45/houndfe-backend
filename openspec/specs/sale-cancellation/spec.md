# Sale Cancellation Specification

## Purpose

Define how the system cancels a CONFIRMED sale that has not been shipped or delivered, restoring stock, recording refund audit rows, and emitting a single `sale.canceled` event. Cancellation MUST be idempotent, RBAC-gated, and safely reject ineligible states.
