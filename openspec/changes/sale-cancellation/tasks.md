...
- [ ] A.1.1 Add `CANCELED` to `SaleStatus` enum in `prisma/schema.prisma:121`
- [ ] A.1.2 Create `SaleCancelReason` enum with 5 values (CUSTOMER_REQUEST, ORDER_ERROR, OUT_OF_STOCK, DUPLICATE_SALE, OTHER)
- [ ] A.1.3 Add cancel metadata to `Sale` model: `canceledAt`, `cancelReason`, `canceledByUserId`, `refunds` relation
- [ ] A.1.4 Create `SaleRefund` model with tenantId, saleId, salePaymentId, method, amountCents, reason, createdAt, indexes
- [ ] A.1.5 Generate Prisma migration with `npx prisma migrate dev --name add-sale-cancellation`

### Phase A.2: Domain Model (TDD)
- [ ] A.2.1 RED: Write spec for `Sale.cancel()` guard: status must be CONFIRMED (`sale.entity.spec.ts`)
- [ ] A.2.2 GREEN: Add `'CANCELED'` to `SaleStatus` type and implement guard in `Sale.cancel()` (`sale.entity.ts:13`)
- [ ] A.2.3 RED: Write spec for delivery guard: SHIPPED/DELIVERED must be rejected
- [ ] A.2.4 GREEN: Add delivery status guard to `Sale.cancel()` throwing `SALE_DELIVERED_CANNOT_CANCEL`
- [ ] A.2.5 RED: Write spec for CREDIT sale: refund = 0, debt cleared
- [ ] A.2.6 GREEN: Implement `Sale.cancel(reason, actor)` with refund computation: CREDIT → 0, else → paidCents
- [ ] A.2.7 Create `SaleCanceledEvent` in `sale.events.ts` with payload: saleId, tenantId, actorId, folio, reason, refundedCents, restockedItems, canceledAt
