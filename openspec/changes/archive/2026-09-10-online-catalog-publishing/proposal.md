...
- **Tenant catalog settings (admin):** new bounded context `src/catalog-settings/`
  ...
  contact) without bloating `src/tenants/`. It exposes `GET/PATCH
/tenants/:tenantId/catalog-settings`. The dedicated `catalog-settings` module owns the
  endpoint; authorization is enforced by JWT, tenant scope, and the dedicated
  `TenantCatalogSettings` permissions.
