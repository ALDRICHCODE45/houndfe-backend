...
## WU7 — Repository wire snapshot equality into writeImpl('GENERIC') (completed)

- **Status:** completed; the change remains **not ready for final verify** until WU8 is complete. WU8 tasks remain unchecked; no WU8 work was done.
- **Completed persisted tasks:** the three WU7 task lines (RED, GREEN, REFACTOR) are marked `[x]` in `tasks.md`.
- **Implementation (inline in `writeImpl`, no extra helper):** after the WU5 forged gate, a GENERIC write against an existing non-DRAFT row performs a separate persisted-items read (`prisma.saleItem.findMany({ where: { saleId: sale.id } })`) and calls the existing WU6 `snapshotItemsEqual`; on mismatch it throws `BusinessRuleViolationError('SALE_NOT_DRAFT','SALE_NOT_DRAFT')` BEFORE `saleItem.deleteMany`, `sale.update`/`sale.create`, `saleItem.createMany`, and all promotion-junction reconciliation (veto/opt-in/applied). Existing-missing (creation preserved) and existing-DRAFT (intent gate accepted) branches skip the compare entirely. The temporary WU6 constructor anchor (`void this.snapshotItemsEqual;`) was removed; the helper comment now records the wiring.
- **Tests:** new `describe('writeImpl GENERIC — snapshot compare wired', …)` with 28 cases — 23 `it.each` parametrized single-column mismatches over `SNAPSHOT_DIFFERING_VALUES` (every mapped projection column incl. `saleId`/`tenantId`) each asserting `SALE_NOT_DRAFT` plus zero writes and zero junction reconciliation; matching-snapshot proceeds with a single `createMany`; missing-existing creates with no persisted-snapshot load (`saleItem.findMany` not called, `findUnique` exactly once, `findFirst` never); existing-DRAFT updates without the compare; sale-row metadata-only change (folio) does not false-positive; mutable-bypass rejects after the incoming side is mutated post-read. Mock access goes through a typed `WritePathMocks` view (`writePathMocks`, `prisma as unknown as WritePathMocks` — no `as any`). The WU5 legitimate GENERIC non-DRAFT test gained the matching persisted-row fixture the wired compare now reads.
- **RED evidence:** baseline 199/199; after RED tests: **24 failed, 199 passed, 223 total** — exactly the 23 parametrized wire-in rejections + 1 mutable-bypass failed (`Expected: BusinessRuleViolationError SALE_NOT_DRAFT, Received: promise resolved`), while the 4 bypass/metadata regression baselines passed.
- **GREEN evidence:** focused run **223/223 passed**. Two GREEN iterations were debugged: (1) the `WritePathMocks` interface originally declared `veto`/`optIn`/`applied` instead of the real mock keys `salePromotionVeto`/`salePromotionOptIn`/`salePromotionApplied` (silent cast rename → `undefined` in `expectNoWrites`), fixed by matching real keys; (2) jest transform cache made one full-suite run report stale failures — cleared cache confirmed 223/223.
- **Broader evidence:** `pnpm test` focused set — repository spec 223/223, `sales.service.spec.ts` 258/258, `sale.entity.spec.ts` 121/121; `pnpm build` passed; `prettier --check` clean on both code files; `git diff --check` clean.
- **Diagnostics discipline:** stash/JSON-diff verified against HEAD baseline — spec eslint 747→747, prod eslint 285→285, **zero new diagnostics** (the pre-existing `as any` fixture baseline remains maintainer-authorized out of scope per WU2/WU4/WU5/WU6 precedent); `nest build` has no `noUnusedLocals` failure after anchor removal.
- **Files changed:** `src/sales/infrastructure/prisma-sale.repository.ts`, `src/sales/infrastructure/prisma-sale.repository.spec.ts`, `openspec/changes/protect-confirmed-sales/tasks.md`, this file. No other file touched (no WU8 `delete()` changes).
- **Budget:** implementation diff vs branch start (HEAD): spec 228+/2−, prod 21+/5− = **256 A+D** (≤ 400 cap; prod A 21 slightly above the 8–15 estimate due to the explanatory comment block, still far under cap). Document lines tracked separately per the tasks.md convention.
- **Rollback boundary:** remove the WU7 wire-in block from `writeImpl`, restore the WU6 constructor anchor, revert the WU7 describe block + `WritePathMocks`/`writePathMocks` helpers + `saleItem.findMany` mock key + WU5-test fixture; WU1–WU6 remain intact.
- **Runtime harness:** N/A — repository unit covered by Jest; no server or background process launched; no acquire/settle ran (parent-owned).
- **R3-TOCTOU correction (parent-authorized bounded fix, correction lineage `review-472e0f2ec04f452f`):** `save()` now wraps `writeImpl(sale,'GENERIC')` in the CLS-propagated `tenantPrisma.runInTransaction` and, inside the tx, first acquires the tenant-qualified row lock `SELECT id FROM sales WHERE id = <sale.id> AND "tenantId" = <tenantId> FOR UPDATE` on the tx-bound client. `saveDraftItems`, services, schema, DTOs, and WU8 behavior untouched.
- **Correction tests:** new `describe('save — transactional sale-row lock (R3-TOCTOU)')` (3 cases: exactly one `runInTransaction` entry; lock SQL tenant-qualified and binding sale id + tenant id; order tx-entry → lock → status read → snapshot read → first destructive write). All pre-existing WU7 mismatch/no-write assertions preserved.
- **Correction verification (observed):** `pnpm test -- src/sales/infrastructure/prisma-sale.repository.spec.ts --runInBand` **226/226 passed**; `pnpm build` passed; `git diff --check` clean; full candidate `git diff --numstat` = **393 A+D** (≤ 400); correction-only vs the frozen 277 A+D baseline = **116 A+D** (≤ 123). Config `tdd: false` — no RED cycle claimed.
- **Correction diagnostics:** the deferred eslint runner reports only the file-wide pre-existing `no-unsafe-*` fixture baseline (verified by probing HEAD: repository.ts carries 285 errors at HEAD, identical count after the correction; prettier clean). No new diagnostic class was introduced.
